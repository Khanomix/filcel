...
class Telegram:
    ...
    UPDATES_CHANNEL = str(env.get('UPDATES_CHANNEL', "Nimleecher"))
    SESSION_NAME = str(env.get('SESSION_NAME', 'FileStream'))
    FORCE_SUB_ID = env.get('FORCE_SUB_ID', None)
    FORCE_SUB = env.get('FORCE_UPDATES_CHANNEL', False)
    FORCE_SUB = True if str(FORCE_SUB).lower() == "true" else False
    SLEEP_THRESHOLD = int(env.get("SLEEP_THRESHOLD", "60"))
    FILE_PIC = env.get('FILE_PIC', "https://graph.org/file/388a6ddebf15ed8036919.jpg")
    START_PIC = env.get('START_PIC', "https://graph.org/file/388a6ddebf15ed8036919.jpg")
